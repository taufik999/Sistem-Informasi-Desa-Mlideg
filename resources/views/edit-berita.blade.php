...
<head>
    ...
    <title>Edit Berita - DESA MLIDEG</title>
    ...
    <main class="main-content">
        ...
        <div class="header">
            <div class="header-title-wrapper">
                ...
                <p>Ubah konten atau status visibilitas artikel Anda.</p>
            </div>
            </div>
        </div>

        <div class="form-card">
            <form action="/admin/berita/{{ $berita['id'] }}/edit" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                    <label class="form-label" for="foto">Foto Sampul (Opsional)</label>
                    @if(isset($berita['foto']) && $berita['foto'])
                        <div style="margin-bottom: 0.5rem; text-align: center; border: 1px dashed #cbd5e1; padding: 0.5rem; border-radius: 8px;">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="judul" name="judul" class="form-control" value="{{ $berita['judul'] }}" required>
                </div>
                
                <div class="form-group">
                    <label class="form-label" for="status">Status Rilis</label>
                    <select id="status" name="status" class="form-control" required>
                        <option value="Published" {{ $berita['status'] == 'Published' ? 'selected' : '' }}>Published (Terbit Secara Publik)</option>
                        <option value="Draft" {{ $berita['status'] == 'Draft' ? 'selected' : '' }}>Draft (Hanya Disimpan)</option>
                    </select>
                </div>
