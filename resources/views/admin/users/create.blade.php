...
@section('header_subtitle', 'Tambahkan admin baru ke sistem')
...
<div class="form-container">
    ...
    <form method="POST" action="/admin/users">
        ...
        <div class="form-group">
            ...
            <input type="text" id="name" name="name" class="form-control" value="{{ old('name') }}" required placeholder="Contoh: Kepala Dusun D">
        </div>

        <div class="form-group">
            <label for="username">Username (Untuk Login)</label>
            <input type="text" id="username" name="username" class="form-control" value="{{ old('username') }}" required placeholder="Contoh: admindusund">
        </div>
        ...
        <div class="form-group">
            ...
            <select id="role" name="role" class="form-control" required>
                ...
                <option value="Super Admin" {{ old('role') == 'Super Admin' ? 'selected' : '' }}>Sekretaris Desa</option>
                <option value="Kepala Dusun Mlideg" {{ old('role') == 'Kepala Dusun Mlideg' ? 'selected' : '' }}>Kepala Dusun Mlideg</option>
                <option value="Kepala Dusun Ngrapah" {{ old('role') == 'Kepala Dusun Ngrapah' ? 'selected' : '' }}>Kepala Dusun Ngrapah</option>
            </select>
            <p style="font-size: 0.8rem; color: #64748b; margin-top: 0.4rem;"><i class="fa-solid fa-circle-info"></i> Sekretaris Desa memiliki akses ke seluruh fitur sistem termasuk pengaturan website dan AHP.</p>
        </div>

        <div style="margin-top: 2rem;">
            <button type="submit" class="btn-submit"><i class="fa-solid fa-save"></i> Simpan Pengguna</button>
        </div>
    </form>
</div>
@endsection
